...
private:
    ...
    //实现16轮加密或解密迭代中的每一轮除去初始置换和逆初始置换的中间操作
...
public:
    ...
    char *pCipherBuffer, int &nCipherBufferLength, char *pKey, int nKeyLength);//加密
    INT32 Decry(char *pCipher, int nCipherBufferLength,
    char *pPlaintextBuffer, int &nPlaintextBufferLength, char *pKey, int nKeyLength);//解密
};
//DES算法中用到的静态数组
//初始置换 IP:
