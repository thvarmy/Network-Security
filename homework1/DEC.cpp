...
INT32 CDesOperate::HandleData(ULONG32 *left, ULONG8 choice) {//执行一次完整的加密或解密操作
    uint32_t *right = &left[1] ;
    uint32_t tmpbuf[2] = { 0 }; 
    for (int  j = 0 ; j < 64 ; j++)//将明文分为64bit的数据块，不够64位的用0补齐。每一轮中，对每一个64bit的数据块，首先进行初始换位，并将数据块分为32bit的两部分
    {
        if (j < 32) 
        {
            if (pc_first[j] > 32)
...
INT32 CDesOperate::MakeData(uint32_t *left ,uint32_t *right ,uint32_t number)//每一轮迭代中的除去初始置换和逆初始置换的中间操作
{
    ...
    int j=0;
    //首先，保持左部不变，将右部由32位扩展成为48位，分别存在两个ULONG32类型的变量里，每个占24bit。
    for (int j = 0 ; j < 48 ; j++)
    {
        if ( j < 24 )
        {
    ...
    //在将右部扩展成为48位之后，与该轮的密钥进行异或操作，由于48位分在一个ULONG32数组中的两个元素中，故要进行两次异或。
    for ( j = 0 ; j < 2 ; j++)
    { 
        exdes_P[j] ^= m_arrOutKey[number][j] ;
    }
    //在异或操作完成之后，对新的48位进行压缩操作，即S盒。将其每取6位，进行一次操作。
    exdes_P[1] >>= 8 ;
    ...
    exdes_P[0] = 0 ;
    exdes_P[1] = 0 ;
    //8个6bit的数据存在ULONG rexpbuf[8]中，然后进行数据压缩操作，每一个6位经过运算之后输出4位，故最终输出的是32位的压缩后的数据。
    *right = 0 ;
    for ( j = 0 ; j < 7 ; j++)
    {
