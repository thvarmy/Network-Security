...
int main()
{
    cout << "Client or Server?" << std::endl;
    char temp;
    cin >> temp;
    if(temp == 's') {
        ...
        close(nListenSocket);
        cout << "server: got connection from " << inet_ntoa(sRemoteAddr.sin_addr) << ", port " << ntohs(sRemoteAddr.sin_port) << ", socket " << nAcceptSocket << std::endl;
